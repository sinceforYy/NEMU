...
word_t raise_intr(word_t NO, vaddr_t epc) {
...
#endif
  printf("NO=%lx, delegS=%x, delegVS=%x, delegM=%x\n", NO, delegS, delegVS, delegM);
#ifdef CONFIG_RV_IMSIC
  printf("mtopi=%d, stopi=%d, vstopi=%d\n", mtopi->iid, stopi->iid, vstopi->iid);
#endif
  ...
  if ((delegVS && !vs_EX_DT) || (virtualInterruptIsHvictlInject && !isNMI)){
    ...
    bool vs_host_no = (vs_guest_no == 2) || (vs_guest_no == 6) || (vs_guest_no == 10);
