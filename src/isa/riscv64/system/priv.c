...
inline void update_stopi() {
  ...
  bool stopi_is_not_zero = !cpu.v && (cpu.mode == MODE_S) && (stopi_gather != 0);
...
static inline word_t csr_read(word_t *src) {
 ...
 if (cpu.v == 1) {
  ...
  else if (is_read(stopi))  { return vstopi->val; }
  else if (is_read(stopei)) { return cpu.xtopei.vstopei; }
...
if (is_read(hip))            { return get_hip(); }
if (is_read(hie))            { return get_hie();}
...
#ifdef CONFIG_RV_IMSIC
  if (is_read(mireg)) {
    word_t iselect = miselect->val;
    if ((iselect > ISELECT_2F_MASK) && (iselect <= ISELECT_3F_MASK) && !(iselect & 0x1)) {
      return cpu.MIprios->iprios[(iselect - ISELECT_2F_MASK - 1) >> 1].val;
    }
    return 0;
  }
  if (is_read(sireg) || is_read(vsireg)) {
    word_t iselect = siselect->val;
    if ((iselect > ISELECT_2F_MASK) && (iselect <= ISELECT_3F_MASK) && !(iselect & 0x1)) {
      return cpu.SIprios->iprios[(iselect - ISELECT_2F_MASK - 1) >> 1].val;
    }
    return 0;
  }
  if (is_read(mtopei)) {
    return cpu.xtopei.mtopei;
  }
  if (is_read(stopei)) {
    return cpu.xtopei.stopei;
  }
  if (is_read(vstopei)) {
    return cpu.xtopei.vstopei;
  }
#endif
...
static inline void csr_write(word_t *dest, word_t src) {
...
#endif // CONFIG_RV_IMSIC
#ifdef CONFIG_RV_IMSIC
    else if (is_write(sireg)) { return; }
    else if (is_write(stopei)) { return; }
#endif
  ...
  else if (is_write(mtopei)) { return; }
  else if (is_write(stopei)) { return; }
  else if (is_write(vstopei)) { return; }
  else if (is_write(vsireg)) { return; }
#endif // CONFIG_RV_IMSIC
#else
  if (is_write(mstatus)) {
#ifndef CONFIG_RVH
...
// as opensbi and linux not support smrnmi, so we default init nmie = 1 and allow nmie set to 0 by software for test
    ...
    *dest = src < CONFIG_TRIGGER_NUM ? src : tselect->val;
    ...
    word_t iselect = miselect->val;
    if ((iselect > ISELECT_2F_MASK) && (iselect <= ISELECT_3F_MASK) && !(iselect & 0x1)) {
      cpu.MIprios->iprios[(iselect - ISELECT_2F_MASK - 1) >> 1].val = src;
    }
  }
  else if (is_write(sireg)) {
    word_t iselect = siselect->val;
    if ((iselect > ISELECT_2F_MASK) && (iselect <= ISELECT_3F_MASK) && !(iselect & 0x1)) {
      cpu.SIprios->iprios[(iselect - ISELECT_2F_MASK - 1) >> 1].val = src;
    }
  }
#endif
#ifdef CONFIG_RV_SSCOFPMF
  ...
  if (is_write(hstatus) || is_write(hvictl)) {
    update_vstopi();
  }
  if (is_write(mip) || is_write(mvip) || is_write(hvip) || is_write(hip) || is_write(sip) || is_write(vsip) ||
